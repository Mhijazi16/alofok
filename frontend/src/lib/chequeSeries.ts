...
/**
 * Increment the trailing run of digits in a cheque number by 1, preserving any
 * non-digit prefix AND the zero-pad width. On carry that needs an extra digit
 * (e.g. "9" → "10", "099" → "100"), the width grows by one as expected.
 *
 * Examples:
 *   "XXXX05" → "XXXX06"
 *   "1009"   → "1010"
 ...
 */
export function nextChequeNumber(current: string): string {
...
export function generateSeries(opts: SeriesOptions): SeriesCheque[] {
  const { startNumber, amount, startDate, intervalMonths } = opts;
  ...
  for (let i = 0; i < count; i++) {
    ...
    checkNumber = nextChequeNumber(checkNumber);
  }
